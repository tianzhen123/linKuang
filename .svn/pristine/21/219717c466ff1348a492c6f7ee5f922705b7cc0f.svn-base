...
summerready = function(){
		viewModel ={
			...
			openWin:function(data){
				console.log(data);
				summer.openWin({
				"id" : "learnOutcomesDetail",
				"url" : "html/partySchool/learnOutcomes/learnOutcomesDetail.html",
				"isKeep" : true,
				"pageParam" : {
					"count" : data,
				}
...
function netService(pageIndex,title){
	var url = ""+getHttpPro()+"xxachive/list?title="+title+"&pageIndex="+pageIndex+"&pageSize=10&sortField=ts&sortDirection=desc";
	summer.get(url, {}, {
