...
summerready = function(){
		viewModel ={
			...
			openWin:function(data){
				...
				summer.openWin({
					"id" : "orgDet",
					"url" : "html/partySchool/organization/orgDet.html",
					"isKeep" : true,
					"pageParam" : {
						"count" : data,
					}
...
function netService(pageIndex,title){
	//$summer.alert(title);
	var url = ""+getHttpPro()+"partyschoolorg/elist?title="+title+"&pageIndex="+pageIndex+"&pageSize=10&sortField=ts&sortDirection=desc";
	summer.get(url, {
