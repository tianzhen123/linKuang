 ...
 *
 */
var time_value;//选择的时间值，点击确定后，用于传给服务器最终日期
var time_value_last;//选择的时间值，点击确定后，用于传给服务器最终日期
var timeOfSelected;//选中的时间
(function ($) {
	var flag = 0;
    ...
    function Scroller(elem, settings) {
        ...
        */
        that.setValue = function (sc, fill, time, temp) {
           ...
           /*1.判断是否点击去顶，flag=1即点击*/
           /*把点击确定前的时间赋值给time_value_last*/
            //flag=1;
            if(flag ==1 ){
            	//alert(flag);
            	time_value_last = time_value;
            	//alert(time_value_last);
            }
            
            /*2.把点击确定之后的时间赋值给time_value*/
           	/*即为选择的时间*/
            //把选择的时间值，赋值到time_value中，点击确定后，用于传给服务器最终日期
          	time_value=v;
           // alert(time_value);
            
            
            
            /*3.判断是否点击确定，flag=1即点击*/
            /*点击确定，检测时间是否改动*/
            if(flag ==1 ){
          		
            	time_check(time_value_last,time_value);//检测时间是否改动（点击确定后，会调用这个方法，时间检测可行）
            	//alert("核对时间");
            	
            }else{//没有点击确定或点击取消，设置选择的时间为默认时间
            	var time_default= time_substr(time_value);
            	setTimeOfDefault(time_default);
            }
        ...
        */
        that.show = function (prevAnim) {
            ...
            html += (s.display != 'inline' ? '<div class="dwbc' + (s.button3 ? ' dwbc-p' : '') + '"><span class="dwbw dwb-s"><span class="dwb" id="definite">' + s.setText + '</span></span>' + (s.button3 ? '<span class="dwbw dwb-n"><span class="dwb" id="month">' + s.button3Text + '</span></span>' : '') + '<span class="dwbw dwb-c"><span class="dwb" id="cancel">' + s.cancelText + '</span></span></div></div>' : '<div class="dwcc"></div>') + '</div></div></div>';
            dw = $(html);
            ...
            }).delegate('.dwb, .dwwb', START_EVENT, function (e) {//三个按键事件
                // Active button
                $(this).addClass('dwb-a');
                //三个按键事件
            }).delegate('#definite', START_EVENT, function (e) {//确定按键事件
                //确定按键事件
               // alert(time_value);
                flag = 1;
        ...
        that.init = function (ss) {
            ...
            if (s.display == 'inline') {
                ...
                if (input && s.showOnFocus) {
                    ...
                    e.readOnly = true;
                    // Init show datewheel
                    elm.bind('focus.dw', function () { that.show(); });//显示时间选框
                }
            }
        };
...
/**
 * 检测时间是否改动（点击确定后，会调用这个方法，时间检测可行）
 * @param {Object} time_value_last 上次的时间
 * @param {Object} time_value 本次的时间
 */
function time_check(time_value_last,time_value){
	//两次时间不同，提交给服务器，进行按条件查询
	if(time_value_last!==time_value){
		var yearmonth= time_substr(time_value);
		setTimeOfSelected(yearmonth);//yearmonth格式20171212
		var time_temp=getTimeOfSelected();//yearmonth格式20171212
		//$summer.alert(typeof(time_temp));
		//alert(yearmonth);
		refresh(0);
		
	}else{
		setTimeOfDefault(time_value);
	}	
}
 ...
 * 
 */
function setTimeOfSelected(time_selected){
	timeOfSelected = time_selected;
}

/**
 *设置没有进行选择时，默认的时间 
 */
function setTimeOfDefault(time_default){
	timeOfSelected = time_default;
}

/**
 *获取选中的时间 
 */
function getTimeOfSelected(){
	return timeOfSelected;
}
...
/**
 * 将获取时间的格式进行更改
 * 2017年01月 转换成 2017-01 
 * 2017年11月 转换成 2017-11 
 */
function time_substr(time){
	var year = time.substr(0,4);
	var month = time.substring(5,7);//2017年01月 转换成 2017-01 
	//var month_temp = Number(month);//2017年01月 转换成 2017-1 
